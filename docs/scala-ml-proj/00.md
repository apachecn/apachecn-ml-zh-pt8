...
[第 9 章](d10834fe-9b85-4ecd-a818-6c223b6c435d.xhtml)、*使用自动编码器和异常检测的欺诈分析*，使用自动编码器和异常检测技术进行欺诈分析。使用的数据集是在 Worldline 和 ULB 大学的机器学习小组的研究合作中收集和分析的欺诈检测数据集。

[第 10 章](ef398231-89c4-48f9-861b-ca50663aa455.xhtml)，*使用递归神经网络进行人体活动识别*，包括另一个端到端项目，展示了如何使用一个名为 LSTM 的 RNN 实现，通过智能手机传感器数据集进行人体活动识别。
